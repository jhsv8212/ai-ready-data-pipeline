...
출력: {카테고리명}_bronze_documents (Streaming Table, 카테고리별로 동적 생성)
  - config.get_category_list()로 S3 Landing Zone 1뎁스 폴더(카테고리명)를 스캔하고,
    카테고리마다 별도 테이블(예: CRM_bronze_documents)을 생성한다.
  - 각 테이블은 해당 카테고리 폴더(s3_landing_path/{카테고리명}/) 아래 파일만 포함한다.
...
def _generate_bronze_documents(category: str):
    ...
    def bronze_documents():
        ...
        category_path = f"{s3_landing_path}{category}/"
        ...
        # S3에서 이 카테고리 폴더의 바이너리 콘텐츠 static 읽기 (각 트리거마다 최신 상태 반영)
        binary_files = (
        ...
        return (
            ...
            # 같은 카테고리 폴더 내 서로 다른 하위 폴더에 동일 파일명이 존재하거나 파일이
            # 재업로드(버전 갱신)되면 동일한 document_id가 재사용된다
            # (pk_{category}_bronze_documents PK 제약과 연동됨).
            .withColumn(
                "document_id",
